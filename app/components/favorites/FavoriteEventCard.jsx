...
export default function FavoriteEventCard({ event, onRemove, type = 'favorite' }) {
  const profile = event.profiles;
  const companyName = profile?.name || event.service_name || 'Event Company';
  const coverImage = event.gallery?.[0] || event.portfolio?.[0] || 'https://images.unsplash.com/photo-1511795409834-ef04bbd61622?auto=format&fit=crop&w=800&q=80';
  const rating = profile?.google_rating || event.average_rating || '4.8';
  ...
  return (
    ...
    >
      ...
      <div className="relative h-48 sm:h-52 w-full overflow-hidden bg-stone-100">
        ...
        {/* Overlay Heart Toggle Button */}
        <div 
          className="absolute top-3.5 right-3.5 z-10"
          onClick={(e) => {
            e.preventDefault();
            e.stopPropagation();
            if (onRemove) onRemove(event.id || event._id);
          }}
        >
          <FavoriteButton 
      ...
      <div className="p-4 sm:p-5 flex flex-col flex-1 justify-between space-y-3">
        <div>
          <div className="flex items-center justify-between gap-2 mb-1">
            ...
            <span className="inline-flex items-center gap-1 bg-amber-50 border border-amber-200/80 text-amber-900 text-[10px] font-black px-2 py-0.5 rounded-full shrink-0">
              <Star className="w-3 h-3 fill-amber-400 text-amber-400" /> Verified
            </span>
          </div>
          <p className="text-xs font-bold text-[#6F4E37]">{event.service_name || event.service_type || 'Event Planner'}</p>
          ...
          <div className="space-y-1.5 mt-2.5">
            <div className="flex items-center gap-2 text-xs font-semibold text-stone-600 line-clamp-1">
              ...
              <span>{event.service_area || event.city || 'Bengaluru'}</span>
            </div>
            <div className="flex items-center gap-2 text-xs font-semibold text-stone-600">
              <Sparkles className="w-3.5 h-3.5 text-[#6F4E37] shrink-0" />
              <span>{event.category ? `${event.category} Category` : `${event.experience_years || 5}+ Years Experience`}</span>
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2 pt-3 border-t border-stone-100 mt-auto">
          <motion.button 
            whileTap={{ scale: 0.94 }}
            onClick={() => onRemove(event.id || event._id)}
          ...
          <Link 
            href={`/customer/event-company/${event.id || event._id}`}
            className="flex-1"
          >
            <motion.div
              whileTap={{ scale: 0.96 }}
